...
const LABEL_WORD = String.raw`(?:api[_ -]?key|apikey|secret|token|password|passwd|pat|private[_ -]?key|client[_ -]?secret|signing[_ -]?secret|webhook(?:[_ -]?url)?|auth[_ -]?token|access[_ -]?token(?:[_ -]?secret)?|refresh[_ -]?token|request[_ -]?token)`;
const SERVICE_NAME = String.raw`(?:posthog|deepgram|google|openai|anthropic|openrouter|xai|groq|perplexity|gitlab|sendgrid|telegram|sentry|supabase|gemini|raindrop)`;
...
const LABEL_AFTER = new RegExp(String.raw`(?<value>${VALUE})\s*(?:-|—|:)\s*(?<label>(?:use\s+this\s+)?(?:api\s+key|key|token|pat|client\s+secret|signing\s+secret|access\s+token|refresh\s+token|request\s+token|webhook))\b`, 'gims');
const SERVICE_LABEL = new RegExp(String.raw`\b(?<label>(?:${SERVICE_NAME})[A-Za-z0-9_ -]{0,25}${LABEL_WORD}|${LABEL_WORD}[A-Za-z0-9_ -]{0,25}(?:${SERVICE_NAME}))\b\s*(?:-|:|=)\s*["'\`]?\s*(?<value>${VALUE})`, 'gims');
const LONG_STANDALONE = /^[A-Za-z0-9][A-Za-z0-9_./+=:@$!%?&%-]{20,}$/;
const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const BAD_LABEL = /(output|input|cache|total|source|prefix|hint|used|match|path|pattern|stress|processed|success|webhookid|tokens\b|nsfilepath|backward compatibility|paramfield|disconnect|duplicate|starturl|key paths|patch|curl -x|mumbai|chhatrapati|shivaji|permissions|api keys page|rotation|behavior|lifecycle|preference|invalidated|leaked|secret isolation|chat-secret)/i;
...
function plausibleValue(raw, { allowShort = false, allowSecretUrl = true } = {}) {
  ...
  if (!digits && !mixedCase) return false;
  if (allowShort && value.length >= 8 && (digits || mixedCase)) return true;
  return value.length >= 16 && (digits || mixedCase) && entropy(value) >= 3.0;
}
...
function detectCredentials(text, { serviceLabels, credentialBlock }) {
  ...
  if (serviceLabels) {
    for (const match of matchAllFresh(SERVICE_LABEL, text)) {
      ...
      const allowShort = /\b(?:posthog|supabase)\b/.test(label);
      const value = match.groups?.value;
