...
const SECRET_HINT = /(sk-|github_pat_|gh[pousr]_|AIza|xox[baprs]-|hooks\.slack\.com\/services|sq0(?:atp|csp)-|shp(?:ss|at|ca|pa)_|eyJ|hf_|npm_|pypi-|api[_ -]?key|apikey|secret|token|password|passwd|pat\b|private[_ -]?key|client[_ -]?(?:secret|id)|app[_ -]?id|signing[_ -]?secret|webhook|auth[_ -]?token|access[_ -]?token|refresh[_ -]?token|request[_ -]?token|posthog|deepgram|google|openai|anthropic|xai|supabase|gemini|raindrop|square|shopify)/i;
