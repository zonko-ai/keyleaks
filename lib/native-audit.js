...
const KNOWN = [
  ['openai_or_router_key', /\bsk-(?:proj-)?[A-Za-z0-9_-]{20,}\b/gms],
...
const SERVICE_LABEL = new RegExp(String.raw`\b(?<label>posthog|deepgram|google|openai|anthropic|xai|supabase|gemini|raindrop)\b\s*(?:-|:|=)\s*["'\`]?\s*(?<value>${VALUE})`, 'gims');
...
const SECRET_HINT = /(sk-|github_pat_|gh[pousr]_|AIza|xox[baprs]-|hooks\.slack\.com\/services|eyJ|hf_|npm_|pypi-|api[_ -]?key|apikey|secret|token|password|passwd|pat\b|private[_ -]?key|client[_ -]?(?:secret|id)|app[_ -]?id|signing[_ -]?secret|webhook|auth[_ -]?token|access[_ -]?token|refresh[_ -]?token|request[_ -]?token|posthog|deepgram|google|openai|anthropic|xai|supabase|gemini|raindrop)/i;
...
function keyTypeFromKind(kind) {
  ...
  const direct = {
    openai_or_router_key: 'OpenAI/OpenRouter',
  ...
  for (const path of paths) {
    ...
    try {
      for await (const line of readLines(path)) {
        ...
        if (obj.type === 'message' && msg.role === 'user') {
          const text = textFromTypedContent(msg.content, 'text');
          if (text.trim()) yield { source: 'pi_session', loc: `${rel}:${lineNo}`, timestamp: obj.timestamp ?? msg.timestamp, text };
        }
      }
    } catch {}
  }
...
async function* iterPiRecordsFast(paths) {
  ...
  for await (const { filePath, lineNo, raw } of iterRgJsonlLines(paths, '"role"\\s*:\\s*"user"')) {
    let obj;
    try { obj = JSON.parse(raw); } catch { continue; }
    const msg = obj && typeof obj.message === 'object' ? obj.message : {};
    if (obj.type !== 'message' || msg.role !== 'user') continue;
    const text = textFromTypedContent(msg.content, 'text');
    if (text.trim()) yield { source: 'pi_session', loc: `${relative(root, filePath)}:${lineNo}`, timestamp: obj.timestamp ?? msg.timestamp, text };
  }
}
...
async function* iterClaudeHistoryRecords() {
  ...
  try {
    for await (const line of readLines(path)) {
      ...
      if (text.trim()) yield { source: 'claude_history', loc: `.claude/history.jsonl:${lineNo}`, timestamp: obj.timestamp, text };
    }
  } catch {}
}
...
function claudeUserTextFromObject(obj) {
  const msg = obj && typeof obj.message === 'object' ? obj.message : {};
  if (obj.type !== 'user' || msg.role !== 'user') return '';
  const content = msg.content;
  if (Array.isArray(content) && !content.some((item) => item && typeof item === 'object' && item.type === 'text')) return '';
  const text = textFromTypedContent(content, 'text');
  const stripped = text.trimStart();
  if (!text.trim()) return '';
  if (CLAUDE_GENERATED_PREFIXES.some((prefix) => stripped.startsWith(prefix)) || stripped.slice(0, 250).includes('<task-id>') || stripped.slice(0, 350).includes('<tool-use-id>')) return '';
  return text;
}
...
async function* iterClaudeProjectRecords(paths) {
  for (const path of paths) {
    ...
    try {
      for await (const line of readLines(path)) {
        lineNo++;
        if ((!line.includes('"role":"user"') && !line.includes('"role": "user"')) || (!line.includes('"type":"user"') && !line.includes('"type": "user"'))) continue;
        let obj;
        try { obj = JSON.parse(line); } catch { continue; }
        const text = claudeUserTextFromObject(obj);
        if (text) yield { source: 'claude_project', loc: `${rel}:${lineNo}`, timestamp: obj.timestamp, text };
      }
    } catch {}
  }
}
...
async function* iterClaudeProjectRecordsFast(paths) {
  ...
  for await (const { filePath, lineNo, raw } of iterRgJsonlLines(paths, '"role"\\s*:\\s*"user"')) {
    if (!raw.includes('"type":"user"') && !raw.includes('"type": "user"')) continue;
    let obj;
    try { obj = JSON.parse(raw); } catch { continue; }
    const text = claudeUserTextFromObject(obj);
    if (text) yield { source: 'claude_project', loc: `${relative(HOME, filePath)}:${lineNo}`, timestamp: obj.timestamp, text };
  }
}
...
async function* iterClaudePasteCacheRecords(paths) {
  for (const path of paths) {
    ...
    try {
      ...
      if (text.trim()) yield { source: 'claude_paste_cache', loc: `${relative(HOME, path)}:1`, timestamp: null, text };
    } catch {}
  }
}
  ...
  for (const name of ['history.jsonl', 'transcription-history.jsonl']) {
    ...
    try {
      for await (const line of readLines(path)) {
        ...
        try { obj = JSON.parse(line); } catch { continue; }
        const text = obj.text || '';
        if (String(text).trim()) yield { source: 'codex_history', loc: `.codex/${name}:${lineNo}`, timestamp: obj.ts ?? obj.createdAtMs, text };
      }
    } catch {}
  }
}
...
async function* iterCodexSessionRecords(paths) {
  for (const path of paths) {
    ...
    try {
      for await (const line of readLines(path)) {
        lineNo++;
        if ((!line.includes('"role":"user"') && !line.includes('"role": "user"')) || (!line.includes('"type":"response_item"') && !line.includes('"type": "response_item"'))) continue;
        let obj;
        try { obj = JSON.parse(line); } catch { continue; }
        const payload = obj && typeof obj.payload === 'object' ? obj.payload : {};
        if (obj.type !== 'response_item' || payload.type !== 'message' || payload.role !== 'user') continue;
        const text = humanishCodexText(textFromTypedContent(payload.content, 'input_text'));
        if (text) yield { source: 'codex_session_humanish', loc: `${rel}:${lineNo}`, timestamp: obj.timestamp, text };
      }
    } catch {}
  }
}
...
async function* iterCodexSessionRecordsFast(paths) {
  ...
  const child = spawn(rg, ['-n', '-H', '--no-heading', '-e', '"role"\\s*:\\s*"user"', ...paths], {
...
async function* iterZedDbRecords(dbFiles) {
  for (const dbPath of dbFiles) {
    ...
    const rows = await sqliteJson(dbPath, `SELECT "${contentCol}" AS text FROM "${String(msgTable).replaceAll('"', '""')}" WHERE role = 'user'`);
    let idx = 0;
    for (const row of rows) {
      idx++;
      const text = row.text || '';
...
async function scanRecords(records, { agentLabel, serviceLabels, credentialBlock, includeEvents, includeDetails, showValues }) {
  let scanned = 0;
  let eventCount = 0;
...
export async function buildReport({ includeEvents = false, includeDetails = false, showValues = false, parallel = false, agents = ALL_AGENT_KEYS, includeInventory = false } = {}) {
...
export { TOOL_LABELS, ALL_AGENT_KEYS };
